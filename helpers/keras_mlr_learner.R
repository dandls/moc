...
get_keras_model = function(layer_size = 0, lr = 3*10^-4, input_shape) {
    ...
    if (layer_size == 0) {
        ...
        ffnet = ffnet %>%
            layer_dense(units = 1L, input_shape = input_shape,  
                activation = "sigmoid")
    } else {
        ffnet%>%
...
trans_target = function(target) {
  y = keras::to_categorical(as.integer(target) - 1)
  y = y[, 1, drop = FALSE]
  return(y)
}
...
initialize_instance = function(inst, data.dir) {
    ...
    sample.id = sample(1:nrow(inst$predictor$data$get.x()), 1)
    ...
    return(list(predictor = inst$predictor, task.id = inst$task.id, 
