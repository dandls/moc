...
get_ice_curve_area = function (instance, features, predictor, param.set, grid.size) {
  ...
  grids = sapply(features, function(feat){
    ...
    as.data.frame(iml:::get.grid.1D(feature = val, grid.size = grid.size,
        type = "equidistant"))
  })
...
plot_ice_curve_area = function(grid, predictor, instance = NULL, x.interest = NULL) {
  ...
  ggMarginal(p, data = predictor$data$get.x(), x = nams[1], y = nams[2], type = "histogram")
}


plot_hv = function(result, ylim = NULL) {
  df = result$log
  ggplot(df, aes(generation, fitness.domHV)) +
    geom_line() +
    theme_bw() +
    ylab("domhv") +
    if (!is.null(ylim)) ylim(ylim[1], ylim[2])
}
