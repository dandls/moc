...
test_that("Counterfactuals only one solutions if target equal prediction", {
  ...
  expect_data_frame(cf$results$counterfactuals, nrow = 1, ncol = ncol(X) + 5)
  expect_data_frame(cf$results$counterfactuals.diff, nrow = 1, ncol = ncol(X) + 5)
...
test_that("Counterfactual solutions if target in infeasible space", {
  ...
  expect_data_frame(cf$results$counterfactuals, min.rows = 1, ncol = ncol(X) + 5)
  expect_data_frame(cf$results$counterfactuals.diff, min.rows = 1, ncol = ncol(X) + 5)
...
test_that("different initialization strategies", {
  ...
  icevar = probs[order(probs, decreasing = TRUE)]
