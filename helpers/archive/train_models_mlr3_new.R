...
packages = c("rstudioapi", "ranger", "mlr3", "mlr3learners", "mlr3pipelines", "mlr3keras", "OpenML", 
  "mlr3misc", "mlr3tuning", "e1071", "stringr", 
...
hyper.pars = list(
  ...
  xgboost = paradox::ParamSet$new(list(
...
hyper.pars$xgboost$trafo = function(x, param_set) {
...
tasks_trained = parallelMap(function(task) {
  ...
  #   lrn("classif.xgboost", nthread = 1L,  predict_type = "prob"), 
  ...
  #     callbacks = list(cb_es(5))))
  # lrn.list[[4]]$man = "mlr3keras::mlr_learners_classif.kerasff"
  #lrn.list[[4]]$param_set$values$callbacks = list(cb_es(5)) # early stopping
  #lrn.list[[5]]$param_set$values$callbacks = list(cb_es(5)) # early stopping
  #lrn.list[[2]]$param_set$values$nthread = 1L # necessary!!
  ...
  learners = list(
    ...
    #     mod = po("scale") %>>%
    #       po("encode") %>>%
    #       po(lrn("classif.keras", model = ffnet, epochs = 100L, predict_type = "prob", 
    #         callbacks = list(cb_es(5))))
    #     mod$keep_results = TRUE
    #     mod
    #   }
